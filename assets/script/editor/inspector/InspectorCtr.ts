...
export default class InspectorCtr extends cc.Component {
    ...
    @property(cc.Node) TimeNode: cc.Node = null;
    ...
    private _timePlay: cc.EditBox = null;
    private _loopToggle: cc.Toggle = null;
    private _repeatCount: cc.EditBox = null;
    private _multiplierLabel: cc.Label = null;

    /** Nút đã chọn trong chế độ xem fsm */
    ...
    protected onLoad() {
        ...
        this._timePlay = this.TimeNode.children[0].getComponent(cc.EditBox);
        this._loopToggle = this.LoopNode.children[0].getComponent(cc.Toggle);
        this._repeatCount = this.LoopNode.children[1].getComponent(cc.EditBox);
    ...
    private showUnitInfo() {
        ...
        if (this._unit instanceof UnitState) {
            ...
            this._repeatCount.string = `${this._unit.state.repeatCount}`;
            this._timePlay.string = this._unit.state.timePlay +"";
    ...
    private onRepeatCountChanged() {
        if (!(this._unit instanceof UnitState)) {
            return;
        }
        let count = parseInt(this._repeatCount.string);
        count = isNaN(count) ? 1 : count;
        this._repeatCount.string = `${count}`;
        this._unit.state.repeatCount = count;
    }
    private onTimeChanged() {
        if (!(this._unit instanceof UnitState)) {
            return;
        }
        this._timePlay.string = this._timePlay.string.replace(/[^.\d]/g, '');
        if (this._timePlay.string === '') {
            return;
        }
        let time = parseFloat(this._timePlay.string);
        this._unit.state.timePlay = isNaN(time) ? 0 : time;
        this._timePlay.string = `${this._unit.state.timePlay}`;
        if(this._unit.state.timePlay > 0){ // neu co time thi luon luon la loop
            this._repeatCount.string = "0";
